...
// Define all data keys for backup
const BACKUP_KEYS = {
  // Main data
...
/**
 * Export all data from localStorage as a backup
 */
...
export async function downloadBackup(
  ...
  try {
    ...
    // Add individual table files for easier inspection
...
export function restoreToLocalStorage(backupData: BackupData): { 
  ...
  // Map backup keys to localStorage keys
...
/**
 * Get backup statistics without creating a backup
 */
export function getBackupStats(): {
  ...
  const lastBackup = localStorage.getItem('choir_last_backup');
...
export function recordBackupTimestamp(): void {
  localStorage.setItem('choir_last_backup', new Date().toISOString());
}
